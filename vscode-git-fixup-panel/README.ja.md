...
直近 20 件から `fixup!` / `squash!` コミットを検出し、すべての修正対象を件名の完全一致で一意に特定でき、対象までの履歴が直線的な場合は、最も古い対象コミットを初期選択します。対象が 20 件より古い場合は、そのコミットを候補に 1 件追加します。別のコミットへの変更やキャンセルも可能で、rebase 実行前の確認は従来どおり表示されます。
...
3. 推奨候補があればそのまま確定するか、別のベースコミットを選択します（そのコミットから `HEAD` までが rebase 対象になります）。
