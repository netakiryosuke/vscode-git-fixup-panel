...
このブランチでは、GitLens Commit Graph（エディタ／サイドバー）の単一コミットの右クリックメニューに、実験的な **Git Fixup: Create Fixup Commit (Beta)** を追加します。コミット選択画面を開かずにそのコミットを対象とするため、直近 20 件より古いコミットも指定できます。
